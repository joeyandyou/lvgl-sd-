...
def change_file(max_num):
    with open("font_change.c", 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    for j in range(0, 65536):
        content_address.append("0xffff")
    data_list_address_f.pop()
    for i in range(0, len(data_list_address_f)):
        num = int(data_list_address_f[i], 16)
        content_address[num] = hex(i)
    print(data_list_address_f[-1])
    content_address = "uint16_t unicode_list_1[0x10000] = {{{}}}".format(", ".join(content_address))
    ...
    new_str_font_dsc = str_font_dsc + (";\nstatic FILE *font_fp = 0;\nstatic uint8_t __g_font_buf[MAX_BUF + 5];\nstatic u8 bin_font_size;\nstatic u8 *font_fp_buf=0;\nstatic int32_t length_list = sizeof(bitmap_list) / sizeof(bitmap_list[0])\n")
    content = content.replace(str_font_dsc, new_str_font_dsc)

    start = content.find('binsearch(const uint16_t *sortedSeq, int seqLength, uint16_t keyData) {')
    end = content.find('return -1;')
    re_str = "binsearch(const uint16_t *sortedSeq, int seqLength, uint16_t keyData) {\n    if(unicode_list_1[keyData]==0xffff)\n    return -1;\n    return unicode_list_1[keyData];\n}"

    str_binsearch = content[start:end+12]
    content = content.replace(str_binsearch, re_str)
